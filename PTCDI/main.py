...
def fit_data():
    ...
    parameters_1, covariance_1 = curve_fit(func, Energy, Counts_1, absolute_sigma=True, bounds=(
        [0.0000001, 2.21, 0.01, 0.00000001, 2.25, 0.01], [1, 2.800000001, 0.5, 1, 2.800000001, 0.5]))
    ...
    textfile = open("Projects/PTCDI/Output/280ps_file.txt", "w")
    for element in fit_y5:
        textfile.write(str(element) + "\n")
    textfile.close()


    textfile = open("Projects/PTCDI/Output/100ps_file.txt", "w")
    for element in fit_y4:
        textfile.write(str(element) + "\n")
    textfile.close()


    textfile = open("Projects/PTCDI/Output/30ps_file.txt", "w")
    for element in fit_y3:
        textfile.write(str(element) + "\n")
    textfile.close()


    textfile = open("Projects/PTCDI/Output/10ps_file.txt", "w")
    for element in fit_y2:
        textfile.write(str(element) + "\n")
    textfile.close()

    textfile = open("Projects/PTCDI/Output/1ps_file.txt", "w")
    ...
    plt.legend()
    plt.savefig("Projects/PTCDI/Output/free_fit.png")
    plt.show()

    original_stdout = sys.stdout
    with open('Projects/PTCDI/Output/parameters-ratio.txt', 'w') as f:
